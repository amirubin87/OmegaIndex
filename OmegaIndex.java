...
public class OmegaIndex {
	...
	public static void main(String[] args) throws IOException {
		...
		CalcOmegaIndex("C:/cygwin/amazon.txt", "C:/cygwin/amazon-betta1.01-0.18.dat", "C:/cygwin/omega.txt");
	}
    ...
    */
	public static Map<Integer, BigDecimal[]> AmountOfSharedCommsToAmountOfPairsOfNodesV2
		(Map<Integer,ArrayList<Integer>> Node2Comm1, Map<Integer,ArrayList<Integer>> Node2Comm2, Map<Integer,ArrayList<Integer>> comm2Nodes1, Map<Integer,ArrayList<Integer>> comm2Nodes2, long M){	    

		Map<Integer,BigDecimal[]> ans = new HashMap<Integer,BigDecimal[]>();
		...
		nodes.addAll(Node2Comm1.keySet());
		nodes.addAll(Node2Comm2.keySet());
		for (int v :nodes){
	        ...
	        for (int u : FilteredNodesReleventToV){
	            ...
	            if (ans.get(amountOfSharedComms1) == null){
	            	BigDecimal[] zeroes= { BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO };            	            
	                ans.put(amountOfSharedComms1, zeroes);
	            }            	            
	            ans.get(amountOfSharedComms1)[0] = ans.get(amountOfSharedComms1)[0].add(BigDecimal.ONE);   
	            ...
	            ans.get(amountOfSharedComms2)[1] = ans.get(amountOfSharedComms2)[1].add(BigDecimal.ONE);	            
	            ...
	            if (amountOfSharedComms1 == amountOfSharedComms2){
	            	ans.get(amountOfSharedComms2)[2] = ans.get(amountOfSharedComms2)[2].add(BigDecimal.ONE);	            	
	            	}
	        }
		}
		...
		ans.get(0)[0] = ans.get(0)[0].add(new BigDecimal(couplesNotCovered));
		ans.get(0)[1] = ans.get(0)[1].add(new BigDecimal(couplesNotCovered));
		ans.get(0)[2] = ans.get(0)[2].add(new BigDecimal(couplesNotCovered));
	...
	public static void CalcOmegaIndex(String listOfNodes1Path, String listOfNodes2Path, String outputPath) throws IOException{    
		...
		int n = Math.max(Node2Comm1.size(), Node2Comm2.size());
		//amount of nodes pairs
		long M = (long)n*(long)(n-1)/2;
		Map<Integer, BigDecimal[]> amountsOfNodesWithSameAmountOfComms =  AmountOfSharedCommsToAmountOfPairsOfNodesV2(Node2Comm1,Node2Comm2, Comm2Nodes1, Comm2Nodes2, M);
		//We and Wu
		BigDecimal sumUnadjustedOmegaIndex = new BigDecimal ("0");
		BigDecimal Wu = new BigDecimal("0");
		BigDecimal sumExpectedOmegaIndex = new BigDecimal ("0");
		BigDecimal We = new BigDecimal("0");
		for (BigDecimal[]val : amountsOfNodesWithSameAmountOfComms.values()){
		  	sumUnadjustedOmegaIndex = sumUnadjustedOmegaIndex.add(val[2]);
		  	sumExpectedOmegaIndex = sumExpectedOmegaIndex.add(val[0].multiply(val[1]));
		}
		
		Wu =  (sumUnadjustedOmegaIndex.divide(new BigDecimal(M), 10, BigDecimal.ROUND_DOWN));
		//We = Math.pow( (Math.sqrt(sumExpectedOmegaIndex)/M) , 2.0);
		We =  (sumExpectedOmegaIndex.divide((new BigDecimal(M).pow(2)), 10, BigDecimal.ROUND_DOWN));
		PrintWriter writer = new PrintWriter(outputPath, "UTF-8");		
		BigDecimal ans = new BigDecimal("0");	
		
		if(!We.equals(BigDecimal.ONE)){
			ans = ((Wu.subtract(We)).divide((BigDecimal.ONE.subtract(We)),10,BigDecimal.ROUND_UP));
		}		
